  ...
  create(context) {
    ...
    function hasIndex(collection, queryFields) {
      ...
      const result = indexes.indexes.some(index => {
        ...
        // Extract field names from both query and index
        const queryFieldNames = queryFields.map(qf => qf.field);
        const indexFieldNames = relevantIndexFields.map(idxf => idxf.fieldPath);
        
        // Check if all query fields are present in the index (in any order for now - simplified)
        // A more sophisticated implementation would check field order and operator compatibility
        const allFieldsPresent = queryFieldNames.every(qfn => indexFieldNames.includes(qfn));
        
        if (!allFieldsPresent) {
          return false;
        }
        
        // Additional checks for array-contains
        for (const queryField of queryFields) {
          if (queryField.operator === 'array-contains' || queryField.operator === 'array-contains-any') {
